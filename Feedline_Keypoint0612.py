...
# 记录每个 batch 的训练时间
					...
					if args.steps_loss > 0 and itr % args.steps_loss == 0 :                                
                        ...
                        # 最后将 PIL 图像转换为 Tensor，并保存到磁盘
							...
							if not (os.path.exists(f'result_save/{args.information}_test_result')):
								os.makedirs(f'result_save/{args.information}_test_result')
							torchvision.utils.save_image(colored_tensor, os.path.join(f'result_save/{args.information}_test_result', f'train_{d}_{imcount}.jpg'))
							imcount += 1
  ...
  #load pretrained model
	...
	if args.loadmodel:
		...
		if args.useautoencoder and args.usetransfer and args.use_backbonebranch :
			...
			predict_dict = torch.load(f'save_drnet/{args.pretrainweight}', map_location=torch.device('cpu'))
			...
			predict_dict = torch.load(f'save_drnet/{args.pretrainweight}')
			model, missing_keys , unexpected_keys = load_my_state_dict(model, predict_dict,args.pretrainweight)
			...
			checkpoint = torch.load(f'save_drnet/{args.pretrainweight}',map_location=torch.device('cpu'))
...
def parse_args():
	...
	parser.add_argument('--basedir', type=str, default='Radar_Keypoint_wh01/')
	...
	parser.add_argument('--num-samples' , type=int, default=262) ## Number of samples from each dataset for train. If empty, consider full dataset.the number of train:val=7:3
	...
	parser.add_argument('--resnet',type=str, default='resnet_50')
	...
	parser.add_argument('--pretrainweight', type=str, default='olddatasets_resnet_50_state_dict_save.pth')#预训练权重位置
	parser.add_argument('--information', type=str, default='lkc0609')  #保存模型名称
	...
	parser.add_argument('--usetransfer' , type=int, default=1)
	parser.add_argument('--useMMDloss' , type=int, default=0)
	parser.add_argument('--paralearnable' , nargs='+', default=['adapt'],help="可学习参数，输入时输入可学习参数的关键字段即可，其余梯度均冻结")
