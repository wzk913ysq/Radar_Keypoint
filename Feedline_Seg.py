...
def train(args, get_dataset, model, enc=False):
	...
	'''for d in datasets:
		for itr, (images, targets) in enumerate(loader_train[d]):
			torchvision.utils.save_image(images, os.path.join('/data1/liwenbo/projects/wang/train_pic/', f'{d}{itr}.jpg'))
	print('---picture saved done---')'''
	...
	if args.model == 'drnet':
		...
		optimizer = Adam(model.optim_parameters(),  weight_decay=1e-4)
	if args.cuda:
		...
		for phase in ['train']:
			...
			for itr in range(n_iters):
				...
				for d in datasets:
					...
					'''image_show=torchvision.transforms.ToPILImage()(targets_l[1,:,:,:].squeeze().float())
					image_show.show()'''
					...
					loss_s = loss_criterion[d](dec_outputs.squeeze() , targets_l)
					loss_s.backward()			
