...
from log_save import save_myfiles
import importlib
...
# 初始化
		...
		if len(args.datasets) == 1 :
			...
			self.metadata = [dinf('ELY', 8, FeedLine_EL, 'datasets_old', (834, 830)),
							dinf('MID', 8, FeedLine_MD, 'datasets_old', (834, 830)),
							dinf('LTE', 8, FeedLine_LT, 'datasets_old', (834, 830)),
							]
...
# 遍历数据集
	...
	loader_val = {dname:DataLoader(dataset_val[dname], num_workers=args.num_workers, batch_size=4, 
...
# 学习率会在每个epoch时更新，更新规则由一个lambda函数确定，该函数接收当前epoch号并返回新的学习率
	...
	#scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch: 1)  ## 不更新学习率
...
# 如果有多个数据集，创建一个EmbeddingLoss模块并使用DataParallel在多个GPU上并行化
	...
	if args.useMMDloss :
		loss_criterion = {key:MMDLoss().cuda() for key in datasets}
	...
	for epoch in range(start_epoch, num_epochs+1):
		spt.setproctitle('Feedline——keypoint——lkc {}/{}'.format(epoch, num_epochs))
...
# 将数据和标签转移到 GPU 上
	...
	# 使用模型生成预测结果，并计算损失
					...
					if args.useMMDloss:
						#print(itr,images_l.shape,targets_l.shape,dec_outputs.shape)
						dec_outputs = dec_outputs.reshape(dec_outputs.shape[0],-1)
						targets_l = targets_l.reshape(targets_l.shape[0],-1)
						#print(f"reshape后 {itr},{images_l.shape},{targets_l.shape},{dec_outputs.shape}")
						loss_s = loss_criterion[d](dec_outputs, targets_l)
						dec_outputs = dec_outputs.reshape(dec_outputs.shape[0],-1,2)
						targets_l = targets_l.reshape(targets_l.shape[0],-1, 2)
						#print(f"还原后 {itr},{images_l.shape},{targets_l.shape},{dec_outputs.shape}")
...
# 记录每个 batch 的训练时间
					...
					if args.steps_loss > 0 and itr % args.steps_loss == 0 :                                
                        ...
                        # 最后将 PIL 图像转换为 Tensor，并保存到磁盘
							...
							if not (os.path.exists(f'/data/wenzhaokun/Radar_Keypoint_wh01/Radar_Keypoint_wh01/result_save/hhc/{args.information}_test_result')):
								os.makedirs(f'/data/wenzhaokun/Radar_Keypoint_wh01/Radar_Keypoint_wh01/result_save/hhc/{args.information}_test_result')
							torchvision.utils.save_image(colored_tensor, os.path.join(f'/data/wenzhaokun/Radar_Keypoint_wh01/Radar_Keypoint_wh01/result_save/hhc/{args.information}_test_result', f'train_{d}_{imcount}.jpg'))
							imcount += 1
...
# 开启模型的验证模式
				...
				lastoutput = torch.zeros(40,8,2).cuda()
        # 遍历每个数据集
				for d in datasets:
					# 初始化时间
					time_taken = []
...
# 设置梯度计算为不可用
 ...
 # 获取模型的输出和损失
							...
							if args.useMMDloss:
								#print(itr,images_l.shape,targets_l.shape,dec_outputs.shape)
								seg_output = seg_output.reshape(seg_output.shape[0],-1)
								targets = targets.reshape(targets.shape[0],-1)
								#print(f"reshape后 {itr},{images_l.shape},{targets_l.shape},{dec_outputs.shape}")
								loss = loss_criterion[d](seg_output, targets)
								seg_output = seg_output.reshape(seg_output.shape[0],-1,2)
								targets = targets.reshape(targets.shape[0],-1, 2)
								#print(f"还原后 {itr},{images_l.shape},{targets_l.shape},{dec_outputs.shape}")
...
#保存模型
#将模型保存到指定路径
		torch.save(model, f'{savedir}/{args.information}_model_save.pth')
		torch.save(model.state_dict(),f'{savedir}/{args.information}_state_dict_save.pth')
#输出每轮epoch的训练时间
...
# 当 epochs_save 大于 0 且 epoch 大于 0，且 epoch 数量是 epochs_save 的倍数时，保存模型
		...
		with open(automated_log_path, "a") as myfile:
			...
			iouTrain = 0
			if len(datasets) > 1:
				myfile.write("\n%d\t\t%.4f\t\t%.4f\t\t%.8f\t\t%s" % (epoch, average_epoch_loss['train'], average_epoch_loss['val'], usedLr,str(args.information )))
...
def load_my_state_dict(model, state_dict,modelname):  #custom function to load model when not all dict keys are there# 定义自定义函数，用于在字典中不包含所有键时加载模型
	mystate_dict = {}
	#print(state_dict)
	...
	for name, param in state_dict.items():
		if "adapter" not in name :
			mystate_dict[name] = param
		# if "base"  in name or "net1" in name or "net2" in name or "pre" in name:#选择转置卷积前的权重进行加载
		# 	mystate_dict[name] = param
		
	missing_keys, unexpected_keys =model.load_state_dict(mystate_dict, strict = False)#加载权重
	

	print(f"Loaded pretrained model {str(modelname)} successfully ")
...
def main(args, get_dataset):
  ...
  #load pretrained model
	...
	if args.loadmodel:
		...
		if args.useautoencoder and args.usetransfer and args.use_backbonebranch :
			raise ValueError("Both useautoencoder and usetransfer cannot be True. Please choose one of them.")
		elif args.useautoencoder :
			model = model_file.Net_withAE(NUM_LABELS , args.em_dim , args.resnet)
			print(f"自编码器的总参数量为{num_params}")
			if args.num_samples is not None :
				weights_X = torch.load(f'AE_save/featureX_dict_beforeconv_weights{args.num_samples}samples.pth', map_location=torch.device('cpu'))
				weights_Y = torch.load(f'AE_save/featureY_dict_beforeconv_weights{args.num_samples}samples.pth', map_location=torch.device('cpu'))
			else:
			...
			for k,v in weights_Y.items():
				if "AE" in k:
					encoder_weights[k] = v
			missing_keys, unexpected_keys =model.load_state_dict(encoder_weights, strict = False)#加载自编码器的权重
			for name, para in model.named_parameters():
				if "AE"  in name:
					para.requires_grad = False#冻结自编码器的权重
			weight_DRNet = torch.load(f"save_drnet/{args.pretrainweight}") 
			model, missing_keys , unexpected_keys = load_my_state_dict(model, weight_DRNet,args.pretrainweight)
		elif args.usetransfer:
			model = model_file.Net_transfer(NUM_LABELS , args.em_dim , args.resnet)
			predict_dict = torch.load(f'/data/wenzhaokun/Radar_Keypoint_wh01/Radar_Keypoint_wh01/save_drnet/{args.pretrainweight}')
			model, missing_keys , unexpected_keys = load_my_state_dict(model, predict_dict,args.pretrainweight)


			para_learnable = 0
			for name, para in model.named_parameters():
				num_params = num_params + para.numel()
				if len(args.paralearnable) > 0 :
					Need_grad = False
					for p in args.paralearnable :
						if p   in name :
							Need_grad = True
							print(f"设置{name}为梯度可更新参数")
					para.requires_grad = Need_grad
				else :
					print(f"未指定冻结部分，默认所有参数均可更新")	
				if para.requires_grad :		
					para_learnable = para.numel() + para_learnable
				# print(f"{name}的参数量为{para.numel()}")
				# if para.requires_grad :
				# 	para_learnable = para.numel() + para_learnable
			print(f"adapterconv的总参数量为{num_params},可学习的参数占总参数的比例为{para_learnable/num_params:.4f}")
		elif args.use_backbonebranch:
			model = model_file.Net_backbonebranch(NUM_LABELS , args.em_dim , args.resnet)
			predict_dict = torch.load(f'/data/lkc/Radar_Keypoint/Radar_Keypoint_wh01/save_drnet/{args.pretrainweight}')
			model, missing_keys , unexpected_keys = load_my_state_dict(model, predict_dict,args.pretrainweight)
			...
			model = model_file.Net_FPN(NUM_LABELS , args.em_dim , args.resnet)
			predict_dict = torch.load(f'/data/lkc/Radar_Keypoint/Radar_Keypoint_wh01/save_drnet/{args.pretrainweight}')
			model, missing_keys , unexpected_keys = load_my_state_dict(model, predict_dict,args.pretrainweight)
			...
			for name, para in model.named_parameters():
				num_params = num_params + para.numel()
				# # if 'adapter' not in name  and 'decoder' not in name:
				# # 	para.requires_grad = False
				#if 'adapter'  not in name and "net7" not in name and "net8" not in name and "net3" not in name and "net4" not in name and "net5" not in name and "net6" not in name:
				if len(args.paralearnable) > 0 :
					Need_grad = False
					for p in args.paralearnable :
						if p   in name :
			...
			#model = torch.load('/data/lkc/Radar_Keypoint/Radar_Keypoint_wh01/save_drnet/{}_model_save.pth'.format(args.pretrainweight))
			checkpoint = torch.load(f'/data/wenzhaokun/Radar_Keypoint_wh01/Radar_Keypoint_wh01/save_drnet/{args.pretrainweight}')
			missing_keys, unexpected_keys = model.load_state_dict(checkpoint)
			print(f"Load {args.pretrainweight} successfully!")
...
def parse_args():
	...
	parser.add_argument('--lr' , type=float, default= 0.001)#学习率0.01 0.005 0.001 0.0005 0.0001
	
	parser.add_argument('--random-rotate' , type=int, default=0)
	parser.add_argument('--random-scale' , type=int, default=0)
	
	parser.add_argument('--num-epochs', type=int, default=500)#训练轮数
	parser.add_argument('--batch-size', type=int, default=5)   
	
	parser.add_argument('--savedir', default='')
	
	parser.add_argument('--datasets' , nargs='+', default=['ELY'])#选用数据集是只用早期还是早中晚都用
	...
	### Optional ######
	parser.add_argument('--loadmodel' , type=int,default=1)#是否加载已训练好模型进行训练
	...
	parser.add_argument('--epochs-save', type=int, default=25)    #You can use this value to save model every X epochs
	parser.add_argument('--iouTrain', action='store_true', default=False) #recommended: False (takes more time to train otherwise)
	parser.add_argument('--iouVal', action='store_true', default=True)  
	parser.add_argument('--resume', action='store_true')    #Use this flag to load last checkpoint for training  
	parser.add_argument('--pretrainweight', type=str, default='黄皓辰_state_dict_save.pth')#预训练权重位置
	
	parser.add_argument('--information', type=str, default='黄皓辰0524')  #保存模型名称
  
	parser.add_argument('--useautoencoder', type=int, default=0) 
	parser.add_argument('--use_backbonebranch' , type=int, default=0) 
	parser.add_argument('--useFPN', type=int, default=0) 
	parser.add_argument('--usetransfer' , type=int, default=0)
	parser.add_argument('--useMMDloss' , type=int, default=0)
	parser.add_argument('--paralearnable' , nargs='+', default=[],help="可学习参数，输入时输入可学习参数的关键字段即可，其余梯度均冻结")
