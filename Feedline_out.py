...
class load_data():
	...
	def __init__(self, args):
		...
		## Origional resolution (834, 830)
		dinf = namedtuple('dinf', ['name', 'n_labels', 'func', 'path', 'size'])
		self.metadata = [dinf('ELY', 2, FeedLine_EL, 'datasets', (417, 415)),
						 dinf('MID', 2, FeedLine_MD, 'datasets', (417, 415)),
						 dinf('LTE', 2, FeedLine_LT, 'datasets', (417, 415)),
						 ]
...
def train(args, get_dataset, model, enc=False):
	...
	#loader_val = {dname:DataLoader(dataset_train[dname], num_workers=args.num_workers, batch_size=args.batch_size, 
	...
	if args.resume:
		...
		val_loss_flag = False
		if args.base_load != '':
			model.module.base = torch.load(args.base_load).module.base
			print(f'base loaded')
			'''model.module.seg[0] = torch.load(args.base_load).module.seg[0]
			model.module.seg[1] = torch.load(args.base_load).module.seg[1]
			model.module.seg[2] = torch.load(args.base_load).module.seg[1]
			print(f'seg loaded')
			model.module.up[0] = torch.load(args.base_load).module.up[0]
			model.module.up[1] = torch.load(args.base_load).module.up[1]
			model.module.up[2] = torch.load(args.base_load).module.up[1]
			print(f'up loaded')'''
	
	scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch: pow((1-((epoch-1)/args.num_epochs)),0.9))  ## scheduler 2
	#loss_criterion = {key:torch.nn.CrossEntropyLoss(ignore_index=0).cuda() for key in datasets}
	...
	for epoch in range(start_epoch, num_epochs+1):
		...
		for phase in ['train']:
			...
			for itr in range(n_iters):
				...
				if args.steps_loss > 0 and (itr % args.steps_loss == 0 or itr == n_iters-1):
					average = {d:np.around(sum(epoch_loss[d]) / len(epoch_loss[d]) , 5) for d in datasets}
		...
		##### Validation ###############
		if (epoch == 1) or (epoch%50==0): ## validation after every 50 epoch
			for phase in ['val']:
				...
				comparison_dir = savedir + '/' + f'val_{epoch}'
					...
					for itr, (images, targets) in enumerate(loader_val[d]):
						...
						with torch.set_grad_enabled(False):
							...
							for bat in range(B):
								...
								torchvision.utils.save_image(colored_tensor, os.path.join(comparison_dir, f'{imcount}_{epoch}_{d}.jpg'))
								imcount += 1
						...
						if args.steps_loss > 0 and (itr % args.steps_loss == 0 or itr == len(loader_val[d])-1):
							average = np.around(np.mean(epoch_val_loss[d]) , 5)
		...
		if epoch % 15 == 0:
			filename = f'{savedir}/model_{epoch}.pth'
			torch.save(model, filename)
...
def parse_args():
	...
	parser.add_argument('--base_load', default='')    #Use this flag to load last checkpoint for training  
